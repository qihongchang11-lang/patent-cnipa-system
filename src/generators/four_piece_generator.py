    ...
    def _generate_claims_with_audit(
        ...
        if (not self.force_rules) and self.llm_client.is_configured():
            ...
            constraint_block = (
                ...
                "\n"
                "【Punctuation Shield：对齐术语一致性校验的正则】\n"
                "TermConsistency 正则会抽取：所述(.+?)[，。；] 与 包括(.+?)[，。；]。\n"
                "因此必须遵守：\n"
                "1) 所述规则：当使用“所述X”时，X 必须为纯名词术语（2~20字），并且紧跟一个中文逗号“，”或句号“。”或分号“；”。\n"
                "   禁止写成“所述X发送/所述X用于...”这种结构（会把动词也被抽取成术语，导致术语污染）。\n"
                "   推荐写法：\"对于所述X，其...\" 或 \"所述X，...\"。\n"
                "2) 包括规则：当使用“包括X”用于触发术语抽取时，X 后必须立刻用中文逗号“，”/句号“。”/分号“；”截断。\n"
                "   不要写“包括A、B和C”。尤其禁止使用顿号“、”做枚举（会被当成一个长术语）。\n"
                "   若要列举多个部件/模块，请使用重复触发：\"包括A，... 还包括B，... 还包括C，...\"。\n"
                "\n"
                "【禁用词替换（避免 BannedWords 命中）】\n"
                "- 第一 -> 首个 / 该（除非引用具体权利要求序号）\n"
                "- 主要 -> 核心\n"
                "- 简单 -> 结构紧凑 / 精简\n"
                "- 方便 -> 易于操作\n"
                "- 彻底 -> 充分\n"
    ...
    def _generate_abstract_with_audit(
        ...
        if (not self.force_rules) and self.llm_client.is_configured():
            ...
            constraint_block = (
                ...
                "\n"
                "【Punctuation Shield：对齐术语一致性校验的正则】\n"
                "如需出现“所述X/包括X”结构，必须保证 X 为纯名词术语且紧跟“，/。/；”截断，避免把动词/长短语当作术语。\n"
                "避免顿号“、”枚举；如必须列举，用“包括A，... 还包括B，...”。\n"
                "\n"
                "【禁用词替换（避免 BannedWords 命中）】\n"
                "- 第一 -> 首个 / 该（除非引用具体权利要求序号）\n"
                "- 主要 -> 核心\n"
