...
async def get_status(job_id: str) -> Dict:
    ...
    # Ensure response is always JSON-serializable (e.g. datetime objects)
    return JSONResponse(content=jsonable_encoder(processing_status[job_id]))
...
async def list_jobs() -> List[Dict]:
    ...
    # Ensure response is always JSON-serializable (e.g. datetime objects)
    return JSONResponse(content=jsonable_encoder(sorted(jobs, key=lambda x: x["created_at"], reverse=True)))
